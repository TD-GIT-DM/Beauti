...
/**
 * Ensure schema + base seed exist. Large aisle files (0004, 0006, 0007, 0008, 0009) are
...
export async function ensureCatalog(db: D1Database): Promise<void> {
  ...
  // Expanded catalog + real-image / real-URL / honest-price UPDATEs are applied
  // via remote migrations / MCP seeding — do not db.exec 0004, 0006, 0007, 0008,
  // or 0009 here (too large → Worker 500).
  const expanded = await db.prepare("SELECT id FROM products WHERE id = ?").bind("mac-ruby-woo").first();
  if (!expanded) {
    console.log(
      "[beauti] expand catalog not applied yet (mac-ruby-woo missing); serving base catalog. Run npm run db:migrate:remote (0004 + 0006 + 0007 + 0008 + 0009).",
    );
    return;
  }
